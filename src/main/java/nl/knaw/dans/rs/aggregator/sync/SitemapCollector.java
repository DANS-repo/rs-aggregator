...
import nl.knaw.dans.rs.aggregator.xml.RsLn;
...
public class SitemapCollector implements RsConstants {
    ...
    public SitemapCollector withConverter(
      LambdaUtil.BiFunction_WithExceptions<URI, HttpResponse, RsRoot, Exception> converter) {
        this.converter = converter;
        return this;
    }

    public SitemapCollector withResourceReader(ResourceReader resourceReader) {
        ...
        for (Result<?> result : currentIndex.getResultMap().values()) {
            if (result.hasErrors()) {
                ...
                for (Throwable error : result.getErrors()) {
                    logger.warn("Result has errors. URI: {}, msg: {}", pathFinder.getCapabilityListUri(),
                      error.getMessage());
                }
            } else {
                analyze(result, pathFinder);
            }
        }
    ...
    private void analyze(Result<?> result, PathFinder pathFinder) {
        if (result.getContent().isPresent()) {
            ...
            if (content instanceof Sitemapindex) {
                ...
                analyzeUrlset(usResult, pathFinder);
            } else {
    ...
    private void analyzeSitemapIndex(Result<Sitemapindex> siResult) {
        ...
        try {
            ...
            if (capa == Capability.RESOURCELIST) {
                ...
                siResult.addError(
                  new RemoteResourceSyncFrameworkException("Unexpected capability on sitemapindex: " + xmlValue));
    ...
    private void analyzeUrlset(Result<Urlset> usResult, PathFinder pathFinder) {
        ...
        try {
            ...
            if (capa == Capability.RESOURCELIST) {
                ...
                analyzeCapabilityList(usResult, pathFinder);
    ...
    private void analyzeResourceList(Result<Urlset> usResult) {
        ...
        if (maybeListAt.isPresent()) {
            ...
            if (listAt.isAfter(ultimateResourceListAt)) {
                ultimateResourceListAt = listAt;
            }
        } else {
            usResult
              .addError(new RemoteResourceSyncFrameworkException("Missing required md:at attribute on resourceList"));
            errorResults.add(usResult);
        ...
        if (rlDate.isAfter(getAsOfDateTime())) {
            ...
            logger
              .debug("Skipping resourceList because completed date {} <= {}: {}", listAt, getAsOfDateTime(), usResult);
        }
    }
    ...
    private void analyzeChangeList(Result<Urlset> usResult) {
        ...
        if (maybeListFrom.isPresent()) {
            ...
            if (listFrom.isAfter(ultimateChangeListFrom)) {
                ultimateChangeListFrom = listFrom;
            }
        } else {
            usResult
              .addError(new RemoteResourceSyncFrameworkException("Missing required md:from attribute on changeList"));
            errorResults.add(usResult);
        ...
        if (clDate.isAfter(getAsOfDateTime())) {
            ...
            for (UrlItem item : changelist.getItemList()) {
                ...
                if (!dateTime.isPresent()) {
                    item.getMetadata().map(rsMd1 -> rsMd1.withFrom(listFrom));
                }
    ...
    private void analyzeCapabilityList(Result<Urlset> usResult, PathFinder pathFinder) {
        Urlset capabilityList = usResult.getContent().orElse(null);
        Optional<RsLn> maybeLink = capabilityList.getLink(RsLn.REL_DESCRIBEDBY);
        if (maybeLink.isPresent()) {
            RsLn rsLn = maybeLink.get();
            URI uri = URI.create(rsLn.getHref());
