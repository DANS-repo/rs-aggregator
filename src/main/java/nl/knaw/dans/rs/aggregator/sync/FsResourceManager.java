...
public class FsResourceManager implements ResourceManager {
    ...
    private PathFinder getPathFinder() {
        if (pathFinder == null) {
            throw new IllegalStateException("Missing PathFinder. No PathFinder was set.");
        }
        return pathFinder;
    }
    ...
    public boolean keepOnly(@Nonnull Set<URI> normalizedURIs) {
        ...
        try {
            ...
            throw new RuntimeException(
              "Could not clear resource directory for " + getPathFinder().getCapabilityListUri(), e);
        }
        return true;
    }
    ...
    public boolean delete(@Nonnull URI normalizedURI) {
        ...
        if (deleted) {
            logger.debug("Deleted {} --> {}", normalizedURI, resourcePath);
        }
        return true;
    }
